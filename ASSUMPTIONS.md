...
- **Arm/consent persistence** is runtime state in `~/.local/share/rewind/state.json` (0600), written only while armed. Disarmed is a hard write gate: no settings flush, pause events, OCR, capture commits, or QML snapshots, including when an archive already exists. Startup recording is `consentAt > 0 && armOnLogin` only; a bare persisted `armed` bit does not resume capture. `arm` is rejected until consent is recorded.
